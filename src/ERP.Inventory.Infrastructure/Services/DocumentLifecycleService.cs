...
public sealed class DocumentLifecycleService : IDocumentLifecycleService
{
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> EditBorrowReturnSelectiveAsync(int id, JsonElement payload, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        foreach (var incomingLine in incomingByInstance.Values)
        {
            ...
            var bin = await _db.BinLocations.AsNoTracking().FirstOrDefaultAsync(x =>
                x.BinCode == incomingLine.Line.TargetBinCode.Trim() &&
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> EditInboundSelectiveAsync(int id, JsonElement payload, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        foreach (var line in request.Lines)
        {
            ...
            var bin = await _db.BinLocations.FirstOrDefaultAsync(x =>
                x.BinCode == line.BinCode.Trim() &&
                x.UsageType == BinLocationUsageType.LocationTracked &&
                x.IsActive, cancellationToken);
        ...
        var cascadeDeps = await _inboundCascadeCleanup.PreviewAsync(new InboundCascadeCleanupRequest
        {
            InboundDocumentId = id,
            RemovedLineIds = removed.Select(x => x.Id).ToArray(),
            ItemInstanceIds = affectedIds,
            DryRun = true
        }, cancellationToken);
        deps.AddRange(cascadeDeps.Reasons);
        if (deps.Count > 0) return await BusinessDependencyFailAsync(deps, "EditInbound", id, user, cancellationToken);
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> EditAdjustmentSelectiveAsync(int id, JsonElement payload, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        foreach (var line in request.Lines)
        {
            ...
            {
                targetBin = await _db.BinLocations.FirstOrDefaultAsync(x =>
                    x.BinCode == line.TargetBinCode.Trim() &&
                    x.UsageType == BinLocationUsageType.LocationTracked &&
                    x.IsActive, cancellationToken);
    ...
    private async Task<ServiceResult<IReadOnlyCollection<(MoveLocationLineRequest Line, ItemInstance Instance, BinLocation TargetBin)>>> PrepareMoveLinesAsync(MoveLocationRequest request, CurrentUserContext user, CancellationToken ct)
    {
        ...
        foreach (var line in request.Lines.Where(x => !string.IsNullOrWhiteSpace(x.ItemCode) && !string.IsNullOrWhiteSpace(x.SerialNumber)))
        {
            ...
            var targetBin = await _db.BinLocations.FirstOrDefaultAsync(x =>
                x.BinCode == line.TargetBinCode.Trim() &&
                x.UsageType == BinLocationUsageType.LocationTracked &&
                x.IsActive, ct);
            if (instance == null) { errors.Add($"Item {line.ItemCode} with serial {line.SerialNumber} not found."); continue; }
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteInboundAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        {
            InboundDocumentId = id,
            RemovedLineIds = document.Lines.Where(x => x.ItemInstanceId.HasValue && instanceIds.Contains(x.ItemInstanceId.Value)).Select(x => x.Id).ToArray(),
            ItemInstanceIds = instanceIds,
            DryRun = true
        }, cancellationToken);
        laterDeps.AddRange(cascadeDeps.Reasons);
        if (laterDeps.Count > 0)
        {
            return await BusinessDependencyFailAsync(laterDeps, "DeleteInbound", id, user, cancellationToken);
        }
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteMoveAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        if (document == null)
        {
            return ServiceResult<DocumentMutationResultDto>.Fail(Text(user.LanguageCode, "move-document-not-found"));
        }
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteAdjustmentAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        if (document == null)
        {
            return ServiceResult<DocumentMutationResultDto>.Fail(Text(user.LanguageCode, "adjustment-document-not-found"));
        }
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteInventoryCheckAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        if (document == null)
        {
            return ServiceResult<DocumentMutationResultDto>.Fail(Text(user.LanguageCode, "inventory-check-document-not-found"));
        }
        ...
        var itemExtraIds = document.Lines
            .Where(x => x.ItemInstanceId.HasValue && x.Result == InventoryCheckLineResult.Extra)
            .Select(x => x.ItemInstanceId!.Value)
            .Distinct()
            .ToArray();

        var unsafeExtraReasons = await FindUnsafeInventoryCheckExtraDeleteReasonsAsync(id, itemExtraIds, user.LanguageCode, cancellationToken);
        if (unsafeExtraReasons.Count > 0)
        {
            return await BusinessDependencyFailAsync(unsafeExtraReasons, "DeleteInventoryCheck", id, user, cancellationToken);
        }

        var rebuildInstanceIds = instanceIds.Except(itemExtraIds).ToArray();
    ...
    private async Task<List<string>> FindUnsafeInventoryCheckExtraDeleteReasonsAsync(int documentId, IReadOnlyCollection<int> itemExtraIds, string language, CancellationToken cancellationToken)
    {
        if (itemExtraIds.Count == 0)
        {
            return new List<string>();
        }

        var locationCreatedByThisDocument = await _db.CurrentItemLocations
            .AsNoTracking()
            .Where(x => itemExtraIds.Contains(x.ItemInstanceId)
                        && x.ReferenceDocumentType == nameof(InventoryCheckDocument)
                        && x.ReferenceDocumentId == documentId)
            .Select(x => x.ItemInstanceId)
            .Distinct()
            .ToArrayAsync(cancellationToken);

        var historyCreatedByThisDocument = await _db.ItemMovementHistories
            .AsNoTracking()
            .Where(x => itemExtraIds.Contains(x.ItemInstanceId)
                        && x.DocumentType == nameof(InventoryCheckDocument)
                        && x.DocumentId == documentId
                        && x.ActionType == MovementActionType.InventoryCheck
                        && x.FromLocationType == null
                        && x.ToLocationType == LocationType.BinLocation)
            .Select(x => x.ItemInstanceId)
            .Distinct()
            .ToArrayAsync(cancellationToken);

        var transactionCreatedByThisDocument = await _db.InventoryTransactions
            .AsNoTracking()
            .Where(x => x.ItemInstanceId.HasValue
                        && itemExtraIds.Contains(x.ItemInstanceId.Value)
                        && x.DocumentType == nameof(InventoryCheckDocument)
                        && x.DocumentId == documentId
                        && x.TransactionType == InventoryTransactionType.InventoryCheck
                        && x.QuantityDelta > 0)
            .Select(x => x.ItemInstanceId!.Value)
            .Distinct()
            .ToArrayAsync(cancellationToken);

        var safeIds = locationCreatedByThisDocument
            .Intersect(historyCreatedByThisDocument)
            .Intersect(transactionCreatedByThisDocument)
            .ToHashSet();

        var reasons = itemExtraIds
            .Where(id => !safeIds.Contains(id))
            .Select(_ => Text(language, "cannot-delete-inventory-check-surplus-source-unverified"))
            .Distinct()
            .ToList();
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteBorrowLendAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        if (latestLog == null)
        {
            return ServiceResult<DocumentMutationResultDto>.Fail("Borrow document has no lend effects to delete.");
        }
        if (latestLog.Action != "BorrowIssue")
        {
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteBorrowReturnLegacyAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        var document = await _db.BorrowDocuments
        .Include(x => x.Lines)
        .FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
        ...
        var returnedLines = document.Lines
            .Where(x => x.IsReturned && x.ReturnedAt.HasValue)
        ...
        // Lấy thời gian trả muộn nhất
        var latestReturnTime = latestLog.Timestamp;

        // Nhóm các dòng thuộc cùng một lần trả (dùng nhiều điều kiện)
        var linesToRevert = returnedLines
            .Where(x =>
                x.ReturnedAt.HasValue &&
                Math.Abs((x.ReturnedAt.Value - latestReturnTime).TotalSeconds) <= 30 && // dung sai 30 giây
                x.UpdatedBy == returnedLines.First().UpdatedBy                     // cùng người thực hiện
            )
            .ToArray();

        var instanceIds = linesToRevert.Select(x => x.ItemInstanceId).Distinct().ToArray();

        // Kiểm tra dependency
        var laterDeps = await FindLaterPhaseDependenciesAsync(
            nameof(BorrowDocument), id, MovementActionType.ReturnBorrowed, instanceIds, cancellationToken);
        ...
        // ====================== XÓA SIDE-EFFECTS ======================
        var fromTime = latestReturnTime.AddSeconds(-30);
        var toTime = latestReturnTime.AddSeconds(30);

        _db.BorrowDocumentLogs.RemoveRange(
            _db.BorrowDocumentLogs.Where(x =>
                x.BorrowDocumentId == id &&
                x.Action == "BorrowReturn" &&
                x.Timestamp >= fromTime && x.Timestamp <= toTime &&
                x.PerformedBy == user.UserName));   // thêm điều kiện người tạo

        _db.InventoryTransactions.RemoveRange(
            _db.InventoryTransactions.Where(x =>
                x.DocumentType == nameof(BorrowDocument) &&
                x.DocumentId == id &&
                x.TransactionType == InventoryTransactionType.BorrowReturn &&
                x.PostedAt >= fromTime && x.PostedAt <= toTime));

        _db.ItemMovementHistories.RemoveRange(
            _db.ItemMovementHistories.Where(x =>
                x.DocumentType == nameof(BorrowDocument) &&
                x.DocumentId == id &&
                x.ActionType == MovementActionType.ReturnBorrowed &&
                x.PerformedAt >= fromTime && x.PerformedAt <= toTime));
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteRepairSendAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        ...
        if (latestLog == null)
        {
            return ServiceResult<DocumentMutationResultDto>.Fail("Repair document has no send effects to delete.");
        }
        if (latestLog.Action != "RepairSend")
        {
        ...
        await RemoveRepairDocumentIfEmptyAsync(document, cancellationToken);
        await _db.SaveChangesAsync(cancellationToken);
        await RebuildLocationTrackedInstancesAsync(instanceIds, cancellationToken);
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteRepairSendLegacyAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        var document = await _db.RepairDocuments.Include(x => x.Lines).FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
        if (document == null)
        {
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> DeleteRepairReceiveLegacyAsync(int id, CurrentUserContext user, CancellationToken cancellationToken)
    {
        var document = await _db.RepairDocuments.Include(x => x.Lines).FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
        if (document == null)
        {
    ...
    {
        var lines = await _db.BorrowDocumentLines.Where(x => x.BorrowDocumentId == document.Id).ToListAsync(cancellationToken);
        var logs = await _db.BorrowDocumentLogs
            .Where(x => x.BorrowDocumentId == document.Id)
            .OrderBy(x => x.Timestamp)
            .ThenBy(x => x.Id)
            .ToListAsync(cancellationToken);
        var histories = await _db.ItemMovementHistories
            .Where(x => x.DocumentType == nameof(BorrowDocument) && x.DocumentId == document.Id)
        ...
        {
            var latest = logs.LastOrDefault(x => x.ItemInstanceId == line.ItemInstanceId);
            if (latest == null)
            {
                _db.BorrowDocumentLines.Remove(line);
                continue;
            }

            var latestHistory = histories.LastOrDefault(x => x.ItemInstanceId == line.ItemInstanceId);
            line.IsReturned = latest.Action == "BorrowReturn";
            line.ReturnedAt = line.IsReturned ? latest.Timestamp : null;
            line.ReturnCondition = line.IsReturned ? ParseBorrowReturnCondition(latest.NewStatus) : null;
            line.TargetBinLocationId = line.IsReturned && latestHistory?.ToLocationType == LocationType.BinLocation ? latestHistory.ToLocationId : null;
            line.UpdatedAt = now;
            line.UpdatedBy = user.UserName;
        }
    }

    private async Task ReplayRepairDocumentLinesAsync(RepairDocument document, CurrentUserContext user, CancellationToken cancellationToken)
    {
        var lines = await _db.RepairDocumentLines.Where(x => x.RepairDocumentId == document.Id).ToListAsync(cancellationToken);
        var logs = await _db.RepairDocumentLogs
            .Where(x => x.RepairDocumentId == document.Id)
            .OrderBy(x => x.Timestamp)
            .ThenBy(x => x.Id)
            .ToListAsync(cancellationToken);
        var histories = await _db.ItemMovementHistories
            .Where(x => x.DocumentType == nameof(RepairDocument) && x.DocumentId == document.Id)
            .OrderBy(x => x.PerformedAt)
            .ThenBy(x => x.Id)
            .ToListAsync(cancellationToken);
        var now = _clock.UtcNow;
        ...
        foreach (var line in lines.ToArray())
        {
            ...
            if (latest == null)
            {
                _db.RepairDocumentLines.Remove(line);
                continue;
            }

            var latestHistory = histories.LastOrDefault(x => x.ItemInstanceId == line.ItemInstanceId);
            line.IsReturned = latest.Action == "RepairReceive";
            line.TargetBinLocationId = line.IsReturned && latestHistory?.ToLocationType == LocationType.BinLocation ? latestHistory.ToLocationId : null;
            line.UpdatedAt = now;
            line.UpdatedBy = user.UserName;
        }
    ...
    private async Task RemoveBorrowDocumentIfEmptyAsync(BorrowDocument document, CancellationToken cancellationToken)
    {
        var hasLogs = await _db.BorrowDocumentLogs.AnyAsync(x => x.BorrowDocumentId == document.Id, cancellationToken);
        if (hasLogs)
        {
            return;
        }
        ...
        var lines = await _db.BorrowDocumentLines.Where(x => x.BorrowDocumentId == document.Id).ToListAsync(cancellationToken);
        _db.BorrowDocumentLines.RemoveRange(lines);
    ...
    private async Task RemoveRepairDocumentIfEmptyAsync(RepairDocument document, CancellationToken cancellationToken)
    {
        ...
        var lines = await _db.RepairDocumentLines.Where(x => x.RepairDocumentId == document.Id).ToListAsync(cancellationToken);
        _db.RepairDocumentLines.RemoveRange(lines);
        _db.RepairDocuments.Remove(document);
    }

    private static BorrowReturnCondition? ParseBorrowReturnCondition(string? value)
    {
        if (Enum.TryParse<BorrowReturnCondition>(value, true, out var condition))
        {
            return condition;
        }

        if (Enum.TryParse<ItemStatus>(value, true, out var status))
        {
            return status switch
            {
                ItemStatus.Damaged => BorrowReturnCondition.Damaged,
                ItemStatus.Lost => BorrowReturnCondition.Lost,
                ItemStatus.Scrapped => BorrowReturnCondition.Scrapped,
                _ => BorrowReturnCondition.Normal
            };
        }
    ...
    private async Task RebuildQuantityInstanceAsync(int warehouseId, int itemId, string snCode, CancellationToken cancellationToken)
    {
        ...
        var quantityInstances = await _db.ItemInstances
            .Where(x => x.ItemId == itemId &&
                        ...
                        x.TrackingType == ItemTrackingType.QuantityOnly)
            .ToListAsync(cancellationToken);
        _db.ItemInstances.RemoveRange(quantityInstances);
    }
    ...
    private async Task<ServiceResult<DocumentMutationResultDto>> BusinessDependencyFailAsync(
    ...
    {
        var list = reasons.Where(x => !string.IsNullOrWhiteSpace(x)).Distinct().ToArray();
        var message = list.FirstOrDefault() ?? "Delete is blocked by downstream dependency.";
        var log = await _errorLog.LogAsync(
            new InvalidOperationException(string.Join(Environment.NewLine, list)),
            new LogErrorContext(
                Module: nameof(DocumentLifecycleService),
                Action: action,
                PayloadJson: JsonSerializer.Serialize(new { documentId, reasons = list }),
                UserId: user.UserId,
                UserName: user.UserName,
                Category: SystemErrorCategory.BusinessDependency,
                StatusCode: 409,
                SafeMessage: message),
            cancellationToken);

        return ServiceResult<DocumentMutationResultDto>.Fail(list, SystemErrorCategory.BusinessDependency.ToString(), log.ErrorCode);
    }
    ...
    private async Task<List<string>> FindLaterPhaseDependenciesAsync(string documentType, int documentId, MovementActionType actionType, IEnumerable<int> instanceIds, CancellationToken cancellationToken)
    {
        ...
        return phaseMarks
            .Where(item => laterCandidates.Any(x =>
                ...
                (x.PerformedAt > item.LastAt ||
                 (x.PerformedAt == item.LastAt && x.Id > item.LastId))))
            .Select(item =>
            {
                var serial = laterCandidates
                    .First(x => x.ItemInstanceId == item.ItemInstanceId)
                    .SerialNumber;

                return $"Item instance {serial} has downstream operations after {actionType}.";
            })
            .ToList();
    }
