    ...
    <div class="col-xl-9">
      <div class="card mb-3"><div class="card-body">
        <div class="row g-3 align-items-end">
          <div class="col-md-4">${UI.select('Import Type','importType', types)}</div>
          ...
          <div class="col-md-3 d-flex gap-2"><button class="btn btn-outline-secondary w-50" id="btnImportTemplate">${UI.t('Template')}</button><button class="btn btn-primary w-50" id="btnImportUpload">${UI.t('Upload File')}</button></div>
        </div>
      </div></div>
...
async function uploadImportFile(){
  ...
  UI.toast(result.message || 'Uploaded');
  await loadImportBatches();
...
$(document).on('click', '.btn-import-validate', async function(){
  ...
  UI.toast(result.message || 'Validated');
  await loadImportBatches();
  await loadImportRows($(this).data('id'));
});
$(document).on('click', '.btn-import-confirm', function(){
  const id = $(this).data('id');
  UI.confirm('Confirm Import', 'Valid rows will be inserted into operational tables.', `<div>Batch ID: <b>${id}</b></div><div>Backend will re-validate before commit.</div>`, async function(){
