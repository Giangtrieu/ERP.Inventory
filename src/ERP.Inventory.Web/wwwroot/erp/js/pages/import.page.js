...
async function loadImportBatches(){
  ...
  const rows = result.data || [];
    ...
    $('#importBatches').html(`<div class="table-wrap"><table class="data-table-detail-exxcel "><thead><tr><th class="px-3">${UI.t('File')}</th><th>${UI.t('Type')}</th><th>${UI.t('Status')}</th><th>${UI.t('Blocking')}/${UI.t('Total')}</th><th></th></tr></thead><tbody>${rows.map(r => `<tr><td class="px-3 fw-semibold" data-id="${r.id}">${UI.esc(r.fileName)}</td><td>${UI.esc(UI.t(r.importType))}</td><td>${UI.esc(UI.enum('ImportBatchStatus', r.status))}</td><td>${r.blockingErrorRows}/${r.totalRows}</td><td><div class="btn-group btn-group-sm"><button class="btn btn-light btn-import-rows" data-id="${r.id}"><i class="bi bi-eye"></i></button><a class="btn btn-outline-secondary" title="${UI.t('Template')}" href="${importDownloadUrl(r.id, 'template')}"><i class="bi bi-file-earmark-spreadsheet"></i></a>${r.hasOriginalFile ? `<a class="btn btn-outline-secondary" title="${UI.t('Original file')}" href="${importDownloadUrl(r.id, 'original')}"><i class="bi bi-download"></i></a>` : ''}<a class="btn btn-outline-secondary" title="${UI.t('Preview file')}" href="${importDownloadUrl(r.id, 'preview')}"><i class="bi bi-table"></i></a>${r.hasValidationErrors ? `<a class="btn btn-outline-danger" title="${UI.t('Validation errors file')}" href="${importDownloadUrl(r.id, 'validation-errors')}"><i class="bi bi-exclamation-triangle"></i></a>` : ''}${r.status === 'Confirmed' ? `<a class="btn btn-outline-success" title="${UI.t('Result file')}" href="${importDownloadUrl(r.id, 'result')}"><i class="bi bi-check2-square"></i></a>` : ''}<button class="btn btn-outline-primary btn-import-validate" data-id="${r.id}" ${r.status === 'Confirmed' ? 'disabled' : ''}>${UI.t('Validate')}</button><button class="btn btn-primary btn-import-confirm" data-id="${r.id}" ${r.status === 'Confirmed' ? 'disabled' : ''}>${UI.t('Confirm')}</button></div></td></tr>`).join('')}</tbody></table></div>`);
...
function importDownloadUrl(id, kind) {
  return `${UI.resolveUrl(`/Import/Download/${id}`)}?kind=${encodeURIComponent(kind)}`;
}
...
$(document).on('click', '.btn-import-confirm', function () {
    ...
    const result = await UI.api(`/Import/Confirm/${id}`, { method: 'POST', data: {}, timeout: 300000 });
        if (!result.success) { UI.showError(UI.resultError(result)); return; }
