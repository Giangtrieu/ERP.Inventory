...
public sealed class SystemController : ManagementBaseController
{
    ...
    public async Task<IActionResult> MigrateDocumentNoItemInstance(CancellationToken cancellationToken)
    {
        ...
        return Ok(new
        {
            ...
            message = $"Successfully updated {affected} items"
        });
    }
    ...
    public async Task<IActionResult> MigrateDocumentLogs(CancellationToken cancellationToken)
    {
        ...
        var borrowLifecycleBatchId = Guid.NewGuid();
        var returnLifecycleBatchId = Guid.NewGuid();    
        foreach (var doc in borrowDocs)
        {
            ...
            foreach (var line in doc.Lines)
            {
                ...
                Db.BorrowDocumentLogs.Add(new ERP.Inventory.Domain.Entities.BorrowDocumentLog
                {
                    ...
                    LifecycleBatchId = borrowLifecycleBatchId,
                ...
                if (line.IsReturned)
                {
                    ...
                    Db.BorrowDocumentLogs.Add(new ERP.Inventory.Domain.Entities.BorrowDocumentLog
                    {
                        ...
                        BorrowDepartment = doc.BorrowDepartment,
                        BorrowerPhone = doc.BorrowerPhone,
                        LifecycleBatchId = returnLifecycleBatchId,
                        DepartmentOwner = doc.ApprovedBy,
        ...
        var inboundLifecycleBatchId = Guid.NewGuid();
        foreach (var doc in inboundDocs)
        {
            ...
            foreach (var line in doc.Lines)
            {
                ...
                Db.InboundDocumentLogs.Add(new ERP.Inventory.Domain.Entities.InboundDocumentLog
                {
                    ...
                    LifecycleBatchId = inboundLifecycleBatchId,
    ...
    [HttpPost("MigrateLifecycleBatchIdLogs")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> MigrateLifecycleBatchIdLogs(CancellationToken cancellationToken)
    {
        int borrowCreated = 0, inboundCreated = 0, adjustCreated = 0;
